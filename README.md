# Hi, I'm Karim 👋

🎓 Life Sciences Graduate → Software Engineering  
💻 Full-Stack Developer (React, Node.js, APIs)  
📍 Based in Canada  

---

## 🚀 About Me

I'm a software engineer with a background in life sciences, currently focused on building real-world, data-driven applications.

I enjoy working on projects that combine **technology, analytics, and user experience**, especially in fintech and healthcare.

Currently, I'm:
- Building full-stack applications with React & Node.js
- Working on API integrations and data-driven features
- Improving UI/UX with responsive design and clean architecture

---

## 🧠 Tech Stack

**Frontend**
- React
- JavaScript (ES6+)
- HTML5, CSS3, SCSS

**Backend**
- Node.js
- Express
- REST APIs

**Tools & Others**
- Git & GitHub
- Postman
- MySQL
- Figma

---

## 📌 Featured Projects

### 🏦 Prism Mortgage Calculator
A full-stack mortgage approval tool that predicts approval likelihood and provides personalized recommendations.

🔗 Live: 
📁 Repo: https://github.com/Kshazly/BrainStation_sprint2_Client.git

**Features:**
- Approval likelihood prediction
- Financial input analysis
- Recommendation engine
- Clean UI based on Figma design

---

### 🎨 Instock 
InStock was a full-stack team project focused on building an Inventory Management System for a Fortune 500 client.

🔗 Live: 
📁 Repo: https://github.com/SarraD-code/instock-client.git

---

### 🌍 Industry Sprint
This project was developed during an industry-focused sprint where the main objective was to address a practical business problem and translate it into a clear digital solution.

🔗 Live:  
📁 Repo: https://github.com/maritzbuchholz/TDFinancialHealth.git

---

## 📈 GitHub Stats

![Karim's GitHub stats](https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true)

---

## 📫 Connect With Me

- LinkedIn: https://linkedin.com/in/yourprofile

<!--
**Kshazly/kshazly** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
